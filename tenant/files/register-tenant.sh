...
parse_cmdline_parameters() {
  ...
  OPTIONS=
  LONGOPTS=install-backend-module:,install-frontend-module:,name:,id:,description:,debug,loadSample,loadReference,upgrade
  ...
  while true; do
    case "$1" in
      --install-backend-module)
        modules="${modules} $2"
	backend_modules="${backend_modules} $2"
        shift 2
        ;;
...
wait_for_backend_modules() {
  ...
  echo "Succeeded waiting for backend modules"
  return 0
}
...
create_admin_workflow() {
  # dont create admin if upgrademode set
  if $upgrademode ; then return 0; fi
  find_module_authtoken  \
  && disable_module_for_tenant $authtoken \
  && create_admin \
  && create_admin_credentials \
  && create_admin_permission "perms.all" \
  && install_modules $authtoken
}
